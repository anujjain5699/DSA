...
TreeNode* preintree(vector<int>&preorder,int psi,
                        int pei,vector<int>&inorder,int isi,int iei)
    {
        ...
        int inx=isi;
        while(inorder[inx]!=preorder[psi])inx++;
        int count=inx-isi;
        root->left=preintree(preorder,psi+1,psi+count,inorder,isi,inx-1);
        root->right=preintree(preorder,psi+count+1,pei,inorder,inx+1,iei);
        return root;
    }
